#!/bin/bash
# Pardus System Health & Journal Log Doctor - kurulum betigi
set -e

AD="pardus-log-doctor"
...
install -m 755 "$KAYNAK/src/pardus_log_doctor.py" /usr/share/$AD/pardus_log_doctor.py

cat > /usr/bin/$AD << 'EOF'
#!/bin/bash
exec python3 /usr/share/pardus-log-doctor/pardus_log_doctor.py "$@"
EOF
