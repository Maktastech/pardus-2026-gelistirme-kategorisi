#!/bin/bash
# Pardus USB Sync-Guard & Buffer Tracker - kurulum betigi
set -e

AD="pardus-usb-syncguard"
...
apt-get install -y python3 python3-pyqt5 udisks2

echo "-> Dosyalar kopyalaniyor..."
install -d /usr/share/$AD
install -m 755 "$KAYNAK/src/pardus_usb_syncguard.py" /usr/share/$AD/pardus_usb_syncguard.py

cat > /usr/bin/$AD << 'EOF'
#!/bin/bash
exec python3 /usr/share/pardus-usb-syncguard/pardus_usb_syncguard.py "$@"
EOF
