#!/bin/bash
# Pardus Local Repository & Cache Cleaner - kurulum betigi
set -e
...
install -m 755 "$KAYNAK/src/pardus_cache_cleaner.py" /usr/share/$AD/pardus_cache_cleaner.py

cat > /usr/bin/$AD << 'EOF'
#!/bin/bash
exec python3 /usr/share/pardus-cache-cleaner/pardus_cache_cleaner.py "$@"
EOF
